...
- now we want to write a single print Method that could able to print `List<Cat>` and `List<Dog>`
  ...
  - you *can't* add a `Cat` to a `List<? extends Animal>` because you don't know it's a `List<Cat>`. You can retrieve a value and know that it will be an `Animal`, but you can't add arbitrary anials.   
